"use client";

import { signIn } from "next-auth/react";

export default function LoginPage() {
  return (
    <div
      className="min-h-screen flex items-center justify-center relative overflow-hidden"
      style={{ background: "oklch(0.10 0 0)" }}
    >
      {/* Grid background */}
      <div
        className="absolute inset-0 opacity-[0.035]"
        style={{
          backgroundImage:
            "linear-gradient(oklch(1 0 0) 1px, transparent 1px), linear-gradient(90deg, oklch(1 0 0) 1px, transparent 1px)",
          backgroundSize: "40px 40px",
        }}
      />

      {/* Amber glow orb */}
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full opacity-[0.06] blur-[120px] pointer-events-none"
        style={{ background: "oklch(0.78 0.18 65)" }}
      />

      <div className="relative w-full max-w-[380px] mx-4">
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-5">
            <div
              className="w-2 h-2 rounded-full animate-pulse"
              style={{ background: "oklch(0.78 0.18 65)" }}
            />
            <span
              className="text-xs tracking-[0.2em] uppercase font-mono"
              style={{ color: "oklch(0.78 0.18 65)" }}
            >
              LeadsWave
            </span>
          </div>
          <h1
            className="text-3xl font-semibold tracking-tight mb-1"
            style={{
              fontFamily: "'Georgia', serif",
              color: "oklch(0.96 0 0)",
              letterSpacing: "-0.02em",
            }}
          >
            Welcome back.
          </h1>
          <p className="text-sm font-mono" style={{ color: "oklch(0.50 0 0)" }}>
            Sign in to continue
          </p>
        </div>

        <div
          className="rounded-xl p-6 space-y-4"
          style={{
            background: "oklch(0.15 0 0)",
            border: "1px solid oklch(1 0 0 / 8%)",
          }}
        >
          <p className="text-sm font-mono text-center" style={{ color: "oklch(0.55 0 0)" }}>
            Sign in with your Google account to access your calendar and outreach tools.
          </p>

          <button
            onClick={() => signIn("google", { callbackUrl: "/" })}
            className="w-full py-2.5 rounded-lg text-sm font-mono tracking-wide transition-all duration-150 flex items-center justify-center gap-3"
            style={{
              background: "oklch(0.78 0.18 65)",
              color: "oklch(0.10 0 0)",
              fontWeight: 600,
            }}
            onMouseEnter={(e) => (e.currentTarget.style.background = "oklch(0.84 0.16 65)")}
            onMouseLeave={(e) => (e.currentTarget.style.background = "oklch(0.78 0.18 65)")}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Continue with Google →
          </button>
        </div>

        <p
          className="text-center text-xs font-mono mt-5"
          style={{ color: "oklch(0.32 0 0)" }}
        >
          LeadsWave · Outbound on autopilot
        </p>
      </div>
    </div>
  );
}
